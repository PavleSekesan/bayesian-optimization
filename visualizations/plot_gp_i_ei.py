...
def main() -> None:
    ...
    lower = mean - 1.96 * std
    upper = mean + 1.96 * std
    ei = expected_improvement_min(mean, variance, float(np.min(y_train)), XI)
    ...
    ax_top.plot(x_plot[:, 0], y_true, color=PALETTE["true"], linewidth=2.3, label="Prava funkcija")
    ax_top.plot(
        x_plot[:, 0],
        mean,
        color=PALETTE["mean"],
    ...
    ax_top.plot(x_plot[:, 0], lower, color=PALETTE["bounds"], linewidth=1.3, label="95% interval poverenja")
    ax_top.plot(x_plot[:, 0], upper, color=PALETTE["bounds"], linewidth=1.3)
    ax_top.fill_between(x_plot[:, 0], lower, upper, color=PALETTE["band"], alpha=0.28, linewidth=0)
    ax_top.scatter(x_train[:, 0], y_train, color=PALETTE["points"], s=30, zorder=5, label="Uzorci funkcije")
    ax_top.legend(loc="upper left")
    ax_top.set_ylabel("f(x)")
    ax_top.set_xlim(x_min, x_max)

    ax_bottom.plot(x_plot[:, 0], ei, color=PALETTE["mean"], linewidth=2.1, label="Očekivano poboljšanje (EI)")
    ax_bottom.fill_between(x_plot[:, 0], 0.0, ei, color=PALETTE["band"], alpha=0.35, linewidth=0)
